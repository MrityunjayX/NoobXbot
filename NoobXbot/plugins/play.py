...
from NoobXbot.NoobXUtilities.database.onoff import (
    is_on_off,
    add_on,
    add_off,
)
from NoobXbot.NoobXUtilities.database.chats import (
    get_served_chats,
    is_served_chat,
    add_served_chat,
    get_served_chats,
...
from NoobXbot.NoobXUtilities.helpers.logger import LOG_CHAT
...
- **Delete message:** For delete message
- **Add new member**: For add my assistant
- **Ban power**: For restrict nigga boiis.
"""
        )
        return
    if not a.can_manage_voice_chats:
        await message.reply_text(
            "I need this power for playing music in your group."
            + "\n❌ MANNAGE VOICE CHAT"
        )
        return
    ...
    try:
        ...
        if b.status == "kicked":
            await message.reply_text(
                f"""
{ASSNAME}(@{ASSUSERNAME}) Get Lost Gay **{chat_title}**
...
"""
        ...
        if message.chat.username:
            try:
                ...
                await message.reply(
                    f"{ASSNAME} Assistant Nigga Joined",
                )
...
"""
    ...
    if audio:
        ...
        mystic = await message.reply_text(
            f"**🔄 Added in my list {username}**"
        )
        if audio.file_size > 157286400:
            await mystic.edit_text("Audio File Must Be Within 150 mb")
            return
...
"""
        ...
        if duration == "None":
            return await mystic.edit_text("Sorry! Live video is not allowed")
        if views == "None":
            return await mystic.edit_text("Sorry! Live video is not allowed")
        ...
        def my_hook(d):
            ...
            if d["status"] == "finished":
                ...
                print(f"[{videoid}] Downloaded| Minute: {taken} seconds")
...
<b>📌 Name: [{title[:25]}]({link})</b>
<b>⏳ Duration:</b> {duration} \n
<b>🎵 Played By: </b>{checking}

<b>#️⃣ Position</b> {position}
""",
            reply_markup=InlineKeyboardMarkup(buttons),
        )
...
async def startyuplay(_, CallbackQuery):
    ...
    if duration == "None":
        return await CallbackQuery.message.reply_text(
            f"Sorry!, Live Videos are not allowed"
        )
...
**Duration must be: {DURATION_LIMIT}**
**Duration:** {duration}
"""
        )
        return
...
async def popat(_,CallbackQuery): 
    ...
    try:
        ...
        return await CallbackQuery.message.edit(f"There is an error\n**The possible reason is**:{e}")       
    if CallbackQuery.from_user.id != int(user_id):
    ...
    if i == 2:
        buttons = search_markup(ID1, ID2, ID3, ID4, ID5, duration1, duration2, duration3, duration4, duration5, user_id, query)
        await CallbackQuery.edit_message_text(
            f"**📌 Please select the song you want to play**¹ <b>{title1}</b>\n  ┗  💡 <u>__[More Information](https://t.me/{BOT_USERNAME}?start=info_{ID1})__</u>\n\n² <b>{title2}</b>\n  ┗  💡 <u>__[More Information](https://t.me/{BOT_USERNAME}?start=info_{ID2})__</u>\n\n³ <b>{title3}</b>\n  ┗  💡 <u>__[More Information](https://t.me/{BOT_USERNAME}?start=info_{ID3})__</u>\n\n⁴ <b>{title4}</b>\n  ┗  💡 <u>__[More Information](https://t.me/{BOT_USERNAME}?start=info_{ID4})__</u>\n\n⁵ <b>{title5}</b>\n  ┗  💡 <u>__[More Information](https://t.me/{BOT_USERNAME}?start=info_{ID5})__</u>",    
            reply_markup=InlineKeyboardMarkup(buttons),
            disable_web_page_preview=True 
        )  
        return
